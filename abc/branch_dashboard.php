...
if (isset($_SESSION['branch_success_message'])) {
    $message = '<div class="alert alert-success alert-dismissible fade show" role="alert">
                    <i class="fas fa-check-circle"></i> ' . $_SESSION['branch_success_message'] . '
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['request_product'])) {
    ...
    addNotification($pdo, null, $supplying_branch, 'branch', 'request', 
        "Product Request from " . $_SESSION['branch_location'], 
        "Request for product: $product_name (Order #$order_id): $request_message",
        "branch_dashboard.php?tab=requests");
    
    $message = '<div class="alert alert-success">Product request sent to ' . $supply_branch['location'] . '!</div>';
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['respond_request'])) {
    ...
    addNotification($pdo, null, $req['requesting_branch'], 'branch', 'request',
        "Product Request " . ucfirst($status),
        "Your request for {$req['product_name']} has been $status. Response: $response_message",
        "branch_dashboard.php?tab=requests");
    
    $message = '<div class="alert alert-success">Request ' . $status . '!</div>';
}

// Get selected order details BEFORE update (to get order_no for redirect)
$selected_order = null;
$selected_products = [];
if ($view_order_id) {
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['update_order'])) {
    ...
    if ($collection_status == 'No') {
        $workflow_status = 'not_collected';
        $packing_status = 'No';
        $dispatch_status = 'No';
    } elseif ($collection_status == 'Yes' && $packing_status == 'No') {
        $workflow_status = 'collecting';
    } elseif ($packing_status == 'Yes' && $dispatch_status == 'No') {
        $workflow_status = 'packing';
    } elseif ($dispatch_status == 'Yes') {
    ...
    $_SESSION['branch_success_message'] = "Order #{$order_no} updated successfully! Status: " . ucfirst(str_replace('_', ' ', $workflow_status));
    
    header('Location: branch_dashboard.php?tab=orders');
    exit;
}

// Get product requests
$received_requests = $pdo->prepare("SELECT pr.*, o.order_no, o.customer_name, b.location as requesting_location, b.branch_code as requesting_code
                                    FROM product_requests pr 
                                    LEFT JOIN shop_orders o ON pr.order_id = o.id
                                    LEFT JOIN branch_managers b ON pr.requesting_branch = b.id
                                    WHERE pr.supplying_branch = ? AND pr.status = 'pending'
                                    ORDER BY pr.created_at DESC");
$received_requests->execute([$branch_id]);
$received_requests = $received_requests->fetchAll();

$sent_requests = $pdo->prepare("SELECT pr.*, o.order_no, o.customer_name, b.location as supplying_location, b.branch_code as supplying_code
                                FROM product_requests pr 
                                LEFT JOIN shop_orders o ON pr.order_id = o.id
                                LEFT JOIN branch_managers b ON pr.supplying_branch = b.id
                                WHERE pr.requesting_branch = ? 
                                ORDER BY pr.created_at DESC");
...
$stats = $pdo->prepare("SELECT 
    SUM(CASE WHEN workflow_status = 'not_collected' OR workflow_status IS NULL THEN 1 ELSE 0 END) as not_collected,
    SUM(CASE WHEN workflow_status = 'collecting' THEN 1 ELSE 0 END) as collecting,
    SUM(CASE WHEN workflow_status = 'packing' THEN 1 ELSE 0 END) as packing,
    SUM(CASE WHEN workflow_status = 'dispatching' THEN 1 ELSE 0 END) as dispatching,
    SUM(CASE WHEN workflow_status = 'completed' OR (packing_status = 'Yes' AND dispatch_status = 'Yes') THEN 1 ELSE 0 END) as completed
    FROM shop_orders WHERE branch_id = ?");
...
<head>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .product-image { width: 60px; height: 60px; object-fit: cover; border-radius: 10px; border: 1px solid #e2e8f0; background: white; }
...
<div class="main-content">
    <div class="top-nav">
        <div class="d-flex justify-content-between align-items-center flex-wrap">
            ...
            <div class="user-info">
                ...
                <button class="btn btn-sm btn-outline-light" onclick="openMessageModal()"><i class="fas fa-comment"></i> Message</button>
    ...
    <div class="main-container">
        ...
        <?php if ($active_tab == 'requests'): ?>
            <div class="row">
                <div class="col-md-6">
                    <h4><i class="fas fa-inbox"></i> Received Requests <?= count($received_requests) > 0 ? '<span class="badge bg-danger">'.count($received_requests).'</span>' : '' ?></h4>
                    <?php if(count($received_requests) > 0): ?>
                        <?php foreach($received_requests as $req): ?>
                        <div class="request-card">
                            <div class="request-header"><strong>From <?= htmlspecialchars($req['requesting_code']) ?> - <?= htmlspecialchars($req['requesting_location']) ?></strong><span class="badge bg-warning ms-2">Pending</span></div>
                            <div class="p-3">
                                <div><strong>Order #<?= htmlspecialchars($req['order_no']) ?></strong> - Customer: <?= htmlspecialchars($req['customer_name']) ?></div>
                                <div>Product: <?= htmlspecialchars($req['product_name']) ?> (SKU: <?= htmlspecialchars($req['sku']) ?>, Size: <?= htmlspecialchars($req['size']) ?>)</div>
                                <div class="mt-2"><em>"<?= htmlspecialchars($req['request_message']) ?>"</em></div>
                                <form method="POST" class="mt-3">
                                    <input type="hidden" name="request_id" value="<?= $req['id'] ?>">
                                    <div class="row g-2">
                                        <div class="col-12"><textarea name="response_message" class="form-control" rows="2" placeholder="Response message..."></textarea></div>
                                        <div class="col-6"><button type="submit" name="respond_request" value="accept" class="btn btn-success w-100" onclick="this.form.action.value='accept'">✓ Accept</button></div>
                                        <div class="col-6"><button type="submit" name="respond_request" value="reject" class="btn btn-danger w-100" onclick="this.form.action.value='reject'">✗ Reject</button></div>
                        ...
                        <?php endforeach; ?>
                    <?php else: ?>
                        <div class="alert alert-secondary">No pending requests received.</div>
                    <?php endif; ?>
                </div>
                <div class="col-md-6">
                    ...
                    <?php if(count($sent_requests) > 0): ?>
                        ...
                        <div class="request-card">
                            <div class="request-header"><strong>To <?= htmlspecialchars($req['supplying_code']) ?> - <?= htmlspecialchars($req['supplying_location']) ?></strong><span class="badge bg-<?= $req['status'] == 'pending' ? 'warning' : ($req['status'] == 'accepted' ? 'success' : 'danger') ?> ms-2"><?= ucfirst($req['status']) ?></span></div>
                            <div class="p-3">
                                <div><strong>Order #<?= htmlspecialchars($req['order_no']) ?></strong> - Customer: <?= htmlspecialchars($req['customer_name']) ?></div>
                                <div>Product: <?= htmlspecialchars($req['product_name']) ?> (SKU: <?= htmlspecialchars($req['sku']) ?>, Size: <?= htmlspecialchars($req['size']) ?>)</div>
                        ...
                        <?php endforeach; ?>
                    <?php else: ?>
                        <div class="alert alert-secondary">No requests sent.</div>
                    <?php endif; ?>
                </div>
            </div>
            
        <?php elseif ($view_order_id && $selected_order): ?>
            <a href="branch_dashboard.php?tab=orders" class="back-btn"><i class="fas fa-arrow-left"></i> Back to Dashboard</a>
            ...
            <div class="order-details-card">
                ...
                <div class="info-section">
                    ...
                    <div class="info-grid">
                        <div class="info-item"><div class="info-label">Order #</div><div class="info-value"><?= htmlspecialchars($selected_order['order_no']) ?></div></div>
                        <div class="info-item"><div class="info-label">KOKO ID</div><div class="info-value"><?= htmlspecialchars($selected_order['koko_online_id'] ?: '-') ?></div></div>
                        ...
                        <div class="product-card">
                            <div class="d-flex gap-3 flex-wrap">
                                ...
                                if($image_url && filter_var($image_url, FILTER_VALIDATE_URL)): ?>
                                    <img src="<?= htmlspecialchars($image_url) ?>" class="product-image" alt="<?= htmlspecialchars($product['product_name']) ?>" 
                                         onerror="this.onerror=null; this.src='https://via.placeholder.com/60x60?text=No+Image'; this.style.objectFit='cover';">
                                <?php else: ?>
                                    ...
                                    <div class="d-flex justify-content-between flex-wrap">
                                        <div>
                                            ...
                                            <small class="text-muted">
                                                <i class="fas fa-tag"></i> Promo: <?= htmlspecialchars($product['promocode']) ?> | 
                                                <i class="fas fa-ruler"></i> Size <?= htmlspecialchars($product['size']) ?> | 
                                        ...
                                        <div class="text-end">
                                            ...
                                            <?php if($product['discount_percent'] > 0): ?><span class="badge bg-warning text-dark">-<?= $product['discount_percent'] ?>%</span><?php endif; ?>
                                        </div>
                                    </div>
                                    <div class="mt-2 pt-2 border-top">
                                        <label class="small fw-bold">Availability:</label>
                                        <div class="radio-group mt-1">
                                            <label class="radio-label"><input type="radio" name="product_avail_<?= $product['id'] ?>" value="available" class="product-avail" data-product="<?= $product['id'] ?>" <?= ($product['product_availability'] != 'not_available') ? 'checked' : '' ?>> <span class="badge bg-success">✓ Available</span></label>
                                        ...
                                        <div class="mt-2"><button type="button" class="btn btn-sm btn-warning" onclick="showRequestModal(<?= $product['id'] ?>, '<?= addslashes($product['product_name']) ?>', '<?= addslashes($product['sku']) ?>', '<?= addslashes($product['size']) ?>', <?= $selected_order['id'] ?>)"><i class="fas fa-exchange-alt"></i> Request from Another Branch</button></div>
                                        <div class="product-reason-div mt-2" id="product-reason-<?= $product['id'] ?>" style="display: <?= ($product['product_availability'] == 'not_available') ? 'block' : 'none' ?>;">
                                            <input type="text" name="product_reason_<?= $product['id'] ?>" class="form-control form-control-sm" placeholder="Reason for unavailability" value="<?= htmlspecialchars($product['product_availability_reason'] ?? '') ?>">
                                        </div>
                                    </div>
                                </div>
                            </div>
                ...
                <div class="status-form">
                    <form method="POST" id="orderForm">
                        ...
                        <div class="info-note mb-3"><i class="fas fa-info-circle me-2"></i> <strong>Workflow:</strong> Product Collection → Packing → Dispatch</div>
                        
                        <div class="row g-4">
                            <div class="col-md-4">
                                <label class="fw-bold mb-2">📦 Product Collection</label>
                                <div class="radio-group" id="collectionGroup">
                                    <label class="radio-label"><input type="radio" name="collection_status" value="Yes" id="collectionYes" <?= ($selected_order['collection_status'] == 'Yes') ? 'checked' : '' ?> onchange="updateWorkflowStatus()"> <span class="badge bg-success">Yes</span></label>
                                    <label class="radio-label"><input type="radio" name="collection_status" value="No" id="collectionNo" <?= ($selected_order['collection_status'] != 'Yes') ? 'checked' : '' ?> onchange="updateWorkflowStatus()"> <span class="badge bg-danger">No</span></label>
                                </div>
                                <input type="text" name="collection_notes" class="form-control mt-2" placeholder="Collection notes" value="<?= htmlspecialchars($selected_order['collection_notes'] ?? '') ?>">
                            </div>
                            <div class="col-md-4">
                                <label class="fw-bold mb-2">📦 Packing</label>
                                <div class="radio-group" id="packingGroup">
                                    <label class="radio-label"><input type="radio" name="packing_status" value="Yes" id="packingYes" <?= ($selected_order['packing_status'] == 'Yes') ? 'checked' : '' ?> onchange="updateWorkflowStatus()"> <span class="badge bg-success">Yes</span></label>
                                    <label class="radio-label"><input type="radio" name="packing_status" value="No" id="packingNo" <?= ($selected_order['packing_status'] != 'Yes') ? 'checked' : '' ?> onchange="updateWorkflowStatus()"> <span class="badge bg-danger">No</span></label>
                                </div>
                            </div>
                            <div class="col-md-4">
                                <label class="fw-bold mb-2">🚚 Dispatch</label>
                                <div class="radio-group" id="dispatchGroup">
                                    <label class="radio-label"><input type="radio" name="dispatch_status" value="Yes" id="dispatchYes" <?= ($selected_order['dispatch_status'] == 'Yes') ? 'checked' : '' ?> onchange="updateWorkflowStatus()"> <span class="badge bg-success">Yes</span></label>
                                    <label class="radio-label"><input type="radio" name="dispatch_status" value="No" id="dispatchNo" <?= ($selected_order['dispatch_status'] != 'Yes') ? 'checked' : '' ?> onchange="updateWorkflowStatus()"> <span class="badge bg-danger">No</span></label>
                                </div>
                            </div>
                        </div>
                        
                        <div class="row g-4 mt-3">
                            <div class="col-md-6"><label class="fw-bold mb-2"><i class="fas fa-barcode"></i> Barcode</label><input type="text" name="barcode_number" class="form-control" value="<?= htmlspecialchars($selected_order['barcode_number'] ?? '') ?>"></div>
                            <div class="col-md-6"><label class="fw-bold mb-2">📄 Invoice</label><input type="text" name="invoice_no" class="form-control" value="<?= htmlspecialchars($selected_order['invoice_no'] ?? '') ?>"></div>
                        </div>
                        
                        <div class="row g-4 mt-3">
                            <div class="col-12"><label class="fw-bold mb-2"><i class="fas fa-sticky-note"></i> Comment</label><input type="text" name="branch_comment" class="form-control" value="<?= htmlspecialchars($selected_order['branch_comment'] ?? '') ?>"></div>
                        </div>
                        
                        <div class="row g-4 mt-3">
                            <div class="col-12">
                                <label class="fw-bold mb-2">Overall Availability</label>
                                <div class="radio-group">
                                    ...
                                    <label class="radio-label"><input type="radio" name="product_availability" value="partial" class="avail-radio" <?= ($selected_order['product_availability'] == 'partial') ? 'checked' : '' ?>> <span class="badge bg-warning">Partially Available</span></label>
                        ...
                        <div id="partial_comment_div" style="display: <?= ($selected_order['product_availability'] == 'partial') ? 'block' : 'none' ?>;"><textarea name="partial_comment" class="form-control mt-2" rows="2" placeholder="Partial availability details..."><?= htmlspecialchars($selected_order['partial_comment'] ?? '') ?></textarea></div>
                        ...
                        <div class="text-end mt-4"><button type="submit" name="update_order" class="btn-confirm"><i class="fas fa-check-circle me-2"></i> Confirm & Share</button></div>
                    </form>
                </div>
            </div>
            ...
            <div class="modal fade" id="requestModal" tabindex="-1"><div class="modal-dialog"><div class="modal-content">
                ...
                <form method="POST"><div class="modal-body">
                    <input type="hidden" name="order_id" id="req_order_id"><input type="hidden" name="product_id" id="req_product_id">
                    <input type="hidden" name="product_name" id="req_product_name"><input type="hidden" name="sku" id="req_sku"><input type="hidden" name="size" id="req_size">
                    <div class="mb-3"><label>Select Branch</label><select name="supplying_branch" class="form-select" required><option value="">Select</option><?php foreach($all_branches as $b): ?><option value="<?= $b['id'] ?>"><?= htmlspecialchars($b['branch_code']) ?> - <?= htmlspecialchars($b['location']) ?></option><?php endforeach; ?></select></div>
                    <div class="mb-3"><label>Message</label><textarea name="request_message" class="form-control" rows="3" required></textarea></div>
                </div><div class="modal-footer"><button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Cancel</button><button type="submit" name="request_product" class="btn btn-primary">Send Request</button></div></form>
            </div></div></div>
            
            <!-- Message Modal -->
            <div class="modal fade" id="messageModal" tabindex="-1"><div class="modal-dialog"><div class="modal-content">
                <div class="modal-header"><h5 class="modal-title"><i class="fas fa-envelope"></i> Send Message</h5><button type="button" class="btn-close" data-bs-dismiss="modal"></button></div>
                <form method="POST"><div class="modal-body">
                    <div class="mb-3"><label>Send To</label><select name="to_type" class="form-select" id="messageToType" onchange="toggleBranchSelect()"><option value="admin">Admin (Head Office)</option><option value="branch">Another Branch</option></select></div>
                    <div class="mb-3" id="branchSelectDiv" style="display:none"><label>Select Branch</label><select name="to_branch" class="form-select"><option value="">Select Branch</option><?php foreach($all_branches as $b): ?><option value="<?= $b['id'] ?>"><?= htmlspecialchars($b['branch_code']) ?> - <?= htmlspecialchars($b['location']) ?></option><?php endforeach; ?></select></div>
                    <div class="mb-3"><label>Subject</label><input type="text" name="subject" class="form-control" required></div>
                    <div class="mb-3"><label>Message</label><textarea name="message_text" class="form-control" rows="4" required></textarea></div>
                </div><div class="modal-footer"><button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Cancel</button><button type="submit" name="send_message" class="btn btn-primary">Send Message</button></div></form>
            </div></div></div>
            
        <?php else: ?>
            <!-- Dashboard View -->
            <div class="stats-grid">
                <div class="stat-card"><div class="stat-icon"><i class="fas fa-shopping-cart"></i></div><div class="stat-number"><?= $total_orders ?></div><div class="stat-label">Total Orders</div></div>
                <div class="stat-card"><div class="stat-icon"><i class="fas fa-clock"></i></div><div class="stat-number"><?= ($stats_data['not_collected'] ?? 0) + ($stats_data['collecting'] ?? 0) ?></div><div class="stat-label">Pending</div></div>
                <div class="stat-card"><div class="stat-icon"><i class="fas fa-box"></i></div><div class="stat-number"><?= $stats_data['packing'] ?? 0 ?></div><div class="stat-label">Packing</div></div>
                <div class="stat-card"><div class="stat-icon"><i class="fas fa-truck"></i></div><div class="stat-number"><?= $stats_data['dispatching'] ?? 0 ?></div><div class="stat-label">Dispatching</div></div>
                <div class="stat-card"><div class="stat-icon"><i class="fas fa-check-circle"></i></div><div class="stat-number"><?= $stats_data['completed'] ?? 0 ?></div><div class="stat-label">Completed</div></div>
            </div>
            
            <div class="search-card">
                <form class="row g-3" onsubmit="return false;"><div class="col-md-9"><label><i class="fas fa-search"></i> Search Orders</label><input type="text" id="searchInput" class="form-control" placeholder="Search by Order Number or Customer..." value="<?= htmlspecialchars($search_query) ?>"></div>
                <div class="col-md-3 d-flex gap-2 align-items-end"><button type="button" class="btn-search w-100" onclick="performSearch()">Search</button><?php if($search_query): ?><a href="branch_dashboard.php?tab=orders" class="btn-clear">Clear</a><?php endif; ?></div></form>
            </div>
            ...
            <div class="orders-table-container">
                ...
                <tbody id="ordersTableBody"><tr><td colspan="7" class="text-center py-5"><div class="spinner-border text-primary"></div></td></tr></tbody>
                </table>
            </div>
        <?php endif; ?>
    </div>
...
<script>
    ...
    function openMessageModal() { new bootstrap.Modal(document.getElementById('messageModal')).show(); }
    ...
    document.querySelectorAll('input[name^="product_reason_"]').forEach(inp => {
        inp.addEventListener('input', function() {
            let match = this.name.match(/product_reason_(\d+)/);
            if(match){ let hr = document.getElementById(`product_reason_hidden_${match[1]}`); if(hr) hr.value = this.value; }
        });
    });
    
    const availRadios = document.querySelectorAll('.avail-radio');
    availRadios.forEach(radio => {
        radio.addEventListener('change', function() {
            let pd = document.getElementById('partial_comment_div'), ud = document.getElementById('unavailability_reason_div');
            if(this.value === 'partial'){ if(pd) pd.style.display = 'block'; if(ud) ud.style.display = 'none'; }
            else if(this.value === 'not_available'){ if(pd) pd.style.display = 'none'; if(ud) ud.style.display = 'block'; }
            else{ if(pd) pd.style.display = 'none'; if(ud) ud.style.display = 'none'; }
        });
    });
    
    async function loadLiveData() {
        if(currentViewOrder || currentTab != 'orders') return;
        try {
            let resp = await fetch(`get_live_data.php?last_update=${lastUpdateTime}`);
            let data = await resp.json();
            if(data.success && data.orders_list){
                let html = '';
                data.orders_list.forEach(o => {
                    let statusClass = o.workflow_status || 'not_collected';
                    let statusText = statusClass.replace('_', ' ');
                    html += `<tr onclick="location.href='?tab=orders&view_order=${o.id}'">`;
                    ...
                    html += `<td>${o.items_count||1}</td>`;
                    html += `<td>Rs.${parseFloat(o.total_amount).toFixed(2)}</td>`;
                    html += `<td><span class="workflow-badge status-${statusClass}">${statusText}</span></td>`;
                    html += `<td><button class="view-btn" onclick="event.stopPropagation();location.href='?tab=orders&view_order=${o.id}'">View</button></td></tr>`;
                });
    ...
    async function markNotificationRead(id) {
        await fetch('mark_notification_read.php', { method: 'POST', headers: { 'Content-Type': 'application/x-www-form-urlencoded' }, body: 'id='+id });
        location.reload();
    }
    
    async function markAllNotificationsRead() {
        await fetch('mark_notification_read.php', { method: 'POST', headers: { 'Content-Type': 'application/x-www-form-urlencoded' }, body: 'all=1' });
    ...
    function startAutoRefresh() { if(!currentViewOrder && currentTab=='orders'){ loadLiveData(); refreshInterval = setInterval(loadLiveData, 10000); } }
    ...
    document.getElementById('packingYes')?.addEventListener('change', updateWorkflowStatus);
    document.getElementById('packingNo')?.addEventListener('change', updateWorkflowStatus);
