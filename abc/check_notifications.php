...
if ($role === 'admin') {
    $stmt = $pdo->prepare("SELECT o.id, o.order_no, o.customer_name, o.updated_at, o.packing_status, o.dispatch_status, o.invoice_no,
                                  b.location as branch_loc
                           FROM shop_orders o 
                           LEFT JOIN branch_managers b ON o.branch_id = b.id 
                           WHERE o.updated_at > FROM_UNIXTIME(?)
                           AND o.branch_id IS NOT NULL
                           AND (o.packing_status = 'Yes' OR o.dispatch_status = 'Yes' OR o.invoice_no IS NOT NULL)
    ...
    $stmt = $pdo->prepare("SELECT COUNT(*) as count FROM shop_orders 
                           ...
                           AND (packing_status = 'Yes' OR dispatch_status = 'Yes' OR invoice_no IS NOT NULL)");
    $stmt->execute([$last_check]);
    ...
    $stmt = $pdo->prepare("SELECT o.id, o.order_no, o.customer_name, o.total_amount, o.created_at, o.status
                           FROM shop_orders o 
                           WHERE o.branch_id = ? 
                           AND o.created_at > FROM_UNIXTIME(?)
                           AND (o.packing_status IS NULL OR o.packing_status = 'No')
