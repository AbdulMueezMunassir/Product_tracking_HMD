...
// Get all showrooms for dropdowns
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['send_message'])) {
    ...
    $from_showroom = ($role == 'branch') ? $branch_id : 0;
...
// Get transfer requests based on role
if ($role == 'admin') {
    $transfers = $pdo->query("SELECT pt.*, 
    ...
    $transfers = $pdo->prepare("SELECT pt.*, 
                                f.location as from_location, f.branch_code as from_code,
                                t.location as to_location, t.branch_code as to_code
                                FROM product_transfers pt 
                                LEFT JOIN branch_managers f ON pt.from_showroom = f.id 
...
if ($role == 'admin') {
    $messages = $pdo->query("SELECT sm.*, 
                             ...
                             LEFT JOIN branch_managers f ON sm.from_showroom = f.id 
                             LEFT JOIN branch_managers t ON sm.to_showroom = t.id 
    ...
    $messages = $pdo->prepare("SELECT sm.*, 
                               ...
                               LEFT JOIN branch_managers f ON sm.from_showroom = f.id 
                               LEFT JOIN branch_managers t ON sm.to_showroom = t.id 
                               WHERE sm.from_showroom = ? OR sm.to_showroom = ?
...
// Get pending request count for notification badge (for branch)
...
<head>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .transfer-card:hover, .message-card:hover { transform: translateY(-2px); box-shadow: 0 5px 20px rgba(0,0,0,0.1); }
        .transfer-header, .message-header {
            background: linear-gradient(135deg, #1e2a3e, #15232e);
            color: white;
            padding: 15px 20px;
        }
...
<div class="sidebar" id="sidebar">
    ...
    <?php else: ?>
        ...
        <?php if($pending_request_count > 0): ?>
            <span class="badge-notify"><?= $pending_request_count ?></span>
        <?php endif; ?>
    <?php endif; ?>
    <hr>
...
<div class="main-content">
    <div class="d-flex justify-content-between align-items-center mb-4">
        <div>
            <h2><i class="fas fa-exchange-alt"></i> Product Transfer</h2>
            <p class="text-muted">Request product transfers between showrooms and communicate with other branches</p>
        </div>
    ...
    <div class="form-card">
        ...
        <div class="p-4">
            <form method="POST">
                <div class="row g-3">
                    <div class="col-md-6">
                        <label class="form-label fw-bold">From Showroom <span class="text-danger">*</span></label>
                        <select name="from_showroom" class="form-select" required>
                            <option value="">Select From Showroom</option>
                            <?php foreach($showrooms as $s): ?>
                                <option value="<?= $s['id'] ?>" <?= ($role != 'admin' && $branch_id == $s['id']) ? 'selected' : '' ?>><?= $s['branch_code'] ?> - <?= $s['location'] ?></option>
                            <?php endforeach; ?>
                        </select>
                    </div>
                    <div class="col-md-6">
                        <label class="form-label fw-bold">To Showroom <span class="text-danger">*</span></label>
                        <select name="to_showroom" class="form-select" required>
                            <option value="">Select To Showroom</option>
                            <?php foreach($showrooms as $s): ?>
                    ...
                    <div class="col-12">
                        ...
                        <textarea name="item_description" class="form-control" rows="3" placeholder="Describe the product to transfer (name, quantity, condition, etc.)"></textarea>
                    </div>
                    <div class="col-md-4">
    ...
    <!-- Send Message to Another Showroom -->
    <div class="form-card">
            ...
            <form method="POST">
                <div class="row g-3">
                    <div class="col-md-6">
                        ...
                        <select name="to_showroom_msg" class="form-select" required>
                            <option value="">Select Showroom</option>
                            <?php foreach($showrooms as $s): ?>
                                <?php if($role != 'admin' && $branch_id == $s['id']) continue; ?>
                                <option value="<?= $s['id'] ?>"><?= $s['branch_code'] ?> - <?= $s['location'] ?></option>
                            <?php endforeach; ?>
                        </select>
                    </div>
                    ...
                    <div class="col-12">
                        <button type="submit" name="send_message" class="btn btn-success"><i class="fas fa-envelope"></i> Send Message</button>
                    </div>
                </div>
            </form>
        </div>
    ...
    <?php if(count($transfers) > 0): ?>
        ...
        <div class="transfer-card">
            <div class="transfer-header">
                <div class="d-flex justify-content-between flex-wrap">
                    <div><strong>Request #<?= $transfer['id'] ?></strong> - <?= $transfer['from_code'] ?> → <?= $transfer['to_code'] ?></div>
                    <span class="status-badge status-<?= $transfer['status'] ?>"><?= ucfirst($transfer['status']) ?></span>
                </div>
            </div>
            <div class="p-3">
                <div class="row">
                    <div class="col-md-6"><small class="text-muted">Requestor:</small><div><strong><?= htmlspecialchars($transfer['requestor']) ?></strong></div></div>
                    <div class="col-md-6"><small class="text-muted">Date:</small><div><?= date('d M Y, h:i A', strtotime($transfer['created_at'])) ?></div></div>
                    <div class="col-12 mt-2"><small class="text-muted">Item Description:</small><div><?= nl2br(htmlspecialchars($transfer['item_description'])) ?></div></div>
                    <div class="col-md-4 mt-2"><small class="text-muted">SKU:</small><div><?= htmlspecialchars($transfer['sku'] ?: '-') ?></div></div>
                    <div class="col-md-4 mt-2"><small class="text-muted">NAV Document:</small><div><?= htmlspecialchars($transfer['nav_document_number'] ?: '-') ?></div></div>
                    <div class="col-md-4 mt-2"><small class="text-muted">Web Order #:</small><div><?= htmlspecialchars($transfer['web_order_number'] ?: '-') ?></div></div>
                    ...
                    <div class="col-12 mt-3">
                        <form method="POST" class="d-flex gap-2">
                            ...
                            <button type="submit" name="update_transfer_status" class="btn btn-sm btn-primary">Update Status</button>
                        </form>
                    </div>
                    <?php endif; ?>
                </div>
        ...
        <?php endforeach; ?>
    <?php else: ?>
        <div class="alert alert-secondary">No transfer requests found.</div>
    <?php endif; ?>
    ...
    <?php if(count($messages) > 0): ?>
        ...
        <div class="message-card <?= ($msg['is_read'] == '0' && $role != 'admin') ? 'message-unread' : '' ?>">
            <div class="message-header" style="background: #2d3e5a;">
                <div class="d-flex justify-content-between flex-wrap">
                    <div><i class="fas fa-envelope"></i> <?= htmlspecialchars($msg['subject'] ?: 'No Subject') ?></div>
                    <div class="small"><?= date('d M Y, h:i A', strtotime($msg['created_at'])) ?></div>
                </div>
            </div>
            <div class="p-3">
                <div class="row">
                    <div class="col-md-6"><small class="text-muted">From:</small><div><strong><?= $msg['from_showroom'] == 0 ? 'Admin' : ($msg['from_code'] . ' - ' . $msg['from_location']) ?></strong></div></div>
                    <div class="col-md-6"><small class="text-muted">To:</small><div><strong><?= $msg['to_code'] . ' - ' . $msg['to_location'] ?></strong></div></div>
                    <div class="col-12 mt-2"><small class="text-muted">Message:</small><div><?= nl2br(htmlspecialchars($msg['message'])) ?></div></div>
                </div>
            </div>
        </div>
        <?php endforeach; ?>
...
<script>
    ...
    document.querySelectorAll('.nav-link').forEach(link => {
        link.addEventListener('click', function() { if (window.innerWidth < 577) toggleSidebar(); });
    });
